...
void UbloxFirmware8::getRosParams()
{
  ...
  getRosUint(
    node_, "gnss.qzss_sig_cfg", qzss_sig_cfg_,
    ublox_msgs::msg::CfgGNSSBlock::SIG_CFG_QZSS_L1CA);

  if (enable_gps_ && !gnss_->isSupported("GPS")) {
    RCLCPP_WARN(
      node_->get_logger(), "gnss.gps is true, but GPS GNSS is not supported by %s",
      "this device");
  }
  if (enable_glonass_ && !gnss_->isSupported("GLO")) {
    RCLCPP_WARN(
      node_->get_logger(), "gnss.glonass is true, but GLONASS is not supported by %s",
      "this device");
  }
  if (enable_galileo_ && !gnss_->isSupported("GAL")) {
    RCLCPP_WARN(
      node_->get_logger(), "gnss.galileo is true, but Galileo GNSS is not supported %s",
      "by this device");
  }
  if (enable_beidou_ && !gnss_->isSupported("BDS")) {
    RCLCPP_WARN(
      node_->get_logger(), "gnss.beidou is true, but Beidou GNSS is not supported %s",
      "by this device");
  }
  if (enable_imes_ && !gnss_->isSupported("IMES")) {
    RCLCPP_WARN(
      node_->get_logger(), "gnss.imes is true, but IMES GNSS is not supported by %s",
      "this device");
  }
  ...
  if (getRosBoolean(node_, "nmea.set")) {
    ...
    if (!getRosUint(node_, "nmea.version", cfg_nmea_.nmea_version)) {
      throw std::runtime_error(
              std::string("Invalid settings: nmea.set is ") +
    ...
    cfg_nmea_.flags |=
      getRosBoolean(node_, "nmea.limit82") ? ublox_msgs::msg::CfgNMEA::FLAGS_LIMIT82 : 0;
    cfg_nmea_.flags |=
      getRosBoolean(node_, "nmea.high_prec") ? ublox_msgs::msg::CfgNMEA::FLAGS_HIGH_PREC : 0;
    // set filter
    cfg_nmea_.filter |=
      getRosBoolean(node_, "nmea.filter.pos") ? ublox_msgs::msg::CfgNMEA::FILTER_POS : 0;
    cfg_nmea_.filter |=
      getRosBoolean(node_, "nmea.filter.msk_pos") ? ublox_msgs::msg::CfgNMEA::FILTER_MSK_POS : 0;
    cfg_nmea_.filter |=
      getRosBoolean(node_, "nmea.filter.time") ? ublox_msgs::msg::CfgNMEA::FILTER_TIME : 0;
    cfg_nmea_.filter |=
      getRosBoolean(node_, "nmea.filter.date") ? ublox_msgs::msg::CfgNMEA::FILTER_DATE : 0;
    cfg_nmea_.filter |=
      getRosBoolean(
      node_,
      "nmea.filter.gps_only") ? ublox_msgs::msg::CfgNMEA::FILTER_GPS_ONLY : 0;
    cfg_nmea_.filter |=
      getRosBoolean(node_, "nmea.filter.track") ? ublox_msgs::msg::CfgNMEA::FILTER_TRACK : 0;
    // set gnssToFilter
    cfg_nmea_.gnss_to_filter |=
      getRosBoolean(
      node_,
      "nmea.gnssToFilter.gps") ? ublox_msgs::msg::CfgNMEA::GNSS_TO_FILTER_GPS : 0;
    cfg_nmea_.gnss_to_filter |=
      getRosBoolean(
      node_,
      "nmea.gnssToFilter.sbas") ? ublox_msgs::msg::CfgNMEA::GNSS_TO_FILTER_SBAS : 0;
    cfg_nmea_.gnss_to_filter |=
      getRosBoolean(
      node_,
      "nmea.gnssToFilter.qzss") ? ublox_msgs::msg::CfgNMEA::GNSS_TO_FILTER_QZSS : 0;
    cfg_nmea_.gnss_to_filter |=
      getRosBoolean(
      node_,
      "nmea.gnssToFilter.glonass") ? ublox_msgs::msg::CfgNMEA::GNSS_TO_FILTER_GLONASS : 0;
    cfg_nmea_.gnss_to_filter |=
      getRosBoolean(
      node_,
      "nmea.gnssToFilter.beidou") ? ublox_msgs::msg::CfgNMEA::GNSS_TO_FILTER_BEIDOU : 0;
...
bool UbloxFirmware8::configureUblox(std::shared_ptr<ublox_gps::Gps> gps)
{
  ...
  if (gps->poll(cfg_gnss)) {
    ...
    RCLCPP_DEBUG(
      node_->get_logger(), "Num. tracking channels in hardware: %i", cfg_gnss.num_trk_ch_hw);
  ...
  for (size_t i = 0; i < cfg_gnss.blocks.size(); i++) {  // NOLINT(modernize-loop-convert)
      ...
      enable_gps_ != (block.flags & ublox_msgs::msg::CfgGNSSBlock::FLAGS_ENABLE))
    {
      ...
      getRosBoolean(
        node_,
        "gnss.sbas") != (block.flags & ublox_msgs::msg::CfgGNSSBlock::FLAGS_ENABLE))
    {
      correct = false;
      cfg_gnss.blocks[i].flags =
        (cfg_gnss.blocks[i].flags & ~ublox_msgs::msg::CfgGNSSBlock::FLAGS_ENABLE) | getRosBoolean(
        node_, "gnss.sbas");
    ...
    {
      RCLCPP_DEBUG(
        node_->get_logger(), "QZSS Configuration is different %u, %u",
  ...
  if (correct) {
    RCLCPP_DEBUG(
      node_->get_logger(), "U-Blox GNSS configuration is correct. GNSS not re-configured.");
  } else if (!gps->configGnss(cfg_gnss, std::chrono::seconds(15))) {
    throw std::runtime_error(
            std::string("Failed to cold reset device ") +
...
void UbloxFirmware8::subscribe(std::shared_ptr<ublox_gps::Gps> gps)
{
  ...
  if (getRosBoolean(node_, "publish.nav.sat")) {
    gps->subscribe<ublox_msgs::msg::NavSAT>(
      [this](const ublox_msgs::msg::NavSAT & m) {nav_sat_pub_->publish(m);},
      kNavSvInfoSubscribeRate);
  }

  // Subscribe to Mon HW
  if (getRosBoolean(node_, "publish.mon.hw")) {
    gps->subscribe<ublox_msgs::msg::MonHW>(
      [this](const ublox_msgs::msg::MonHW & m) {mon_hw_pub_->publish(m);},
      1);
  }

  // Subscribe to RTCM messages
  if (getRosBoolean(node_, "publish.rxm.rtcm")) {
    gps->subscribe<ublox_msgs::msg::RxmRTCM>(
      [this](const ublox_msgs::msg::RxmRTCM & m) {rxm_rtcm_pub_->publish(m);},
      1);
  }
}
