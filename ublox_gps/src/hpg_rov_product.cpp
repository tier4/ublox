...
HpgRovProduct::HpgRovProduct(
  ...
  rclcpp::Node * node)
: dgnss_mode_(ublox_msgs::msg::CfgDGNSS::DGNSS_MODE_RTK_FIXED), nav_rate_(nav_rate), updater_(
    updater), node_(node)
{
...
void HpgRovProduct::subscribe(std::shared_ptr<ublox_gps::Gps> gps)
{
  ...
  gps->subscribe<ublox_msgs::msg::NavRELPOSNED>(
    std::bind(
...
{
  freq_rtcm_ = std::make_unique<UbloxTopicDiagnostic>(
    std::string("rxmrtcm"),
    kRtcmFreqMin, kRtcmFreqMax,
    kRtcmFreqTol, kRtcmFreqWindow, updater_);
  updater_->add(
    "Carrier Phase Solution", this,
