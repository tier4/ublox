...
void UbloxFirmware7::getRosParams()
{
  ...
  if (enable_gps_ && !gnss_->isSupported("GPS")) {
    RCLCPP_WARN(
      node_->get_logger(), "gnss/gps is true, but GPS GNSS is not supported by this device");
  }
  if (enable_glonass_ && !gnss_->isSupported("GLO")) {
    RCLCPP_WARN(
      node_->get_logger(), "gnss/glonass is true, but GLONASS is not %s",
  ...
  if (getRosBoolean(node_, "nmea.set")) {
    ...
    // set filter
    cfg_nmea_.filter |=
      getRosBoolean(node_, "nmea.filter.pos") ? ublox_msgs::msg::CfgNMEA7::FILTER_POS : 0;
    cfg_nmea_.filter |=
      getRosBoolean(node_, "nmea.filter.msk_pos") ? ublox_msgs::msg::CfgNMEA7::FILTER_MSK_POS : 0;
    cfg_nmea_.filter |=
      getRosBoolean(node_, "nmea.filter.time") ? ublox_msgs::msg::CfgNMEA7::FILTER_TIME : 0;
    cfg_nmea_.filter |=
      getRosBoolean(node_, "nmea.filter.date") ? ublox_msgs::msg::CfgNMEA7::FILTER_DATE : 0;
    cfg_nmea_.filter |=
      getRosBoolean(
      node_,
      "nmea.filter.gps_only") ? ublox_msgs::msg::CfgNMEA7::FILTER_GPS_ONLY : 0;
    cfg_nmea_.filter |=
      getRosBoolean(node_, "nmea.filter.track") ? ublox_msgs::msg::CfgNMEA7::FILTER_TRACK : 0;
    // set gnssToFilter
    cfg_nmea_.gnss_to_filter |=
      getRosBoolean(
      node_,
      "nmea.gnssToFilter.gps") ? ublox_msgs::msg::CfgNMEA7::GNSS_TO_FILTER_GPS : 0;
    cfg_nmea_.gnss_to_filter |=
      getRosBoolean(
      node_,
      "nmea.gnssToFilter.sbas") ? ublox_msgs::msg::CfgNMEA7::GNSS_TO_FILTER_SBAS : 0;
    cfg_nmea_.gnss_to_filter |=
      getRosBoolean(
      node_,
      "nmea.gnssToFilter.qzss") ? ublox_msgs::msg::CfgNMEA7::GNSS_TO_FILTER_QZSS : 0;
    cfg_nmea_.gnss_to_filter |=
      getRosBoolean(
      node_,
      "nmea.gnssToFilter.glonass") ? ublox_msgs::msg::CfgNMEA7::GNSS_TO_FILTER_GLONASS : 0;
...
bool UbloxFirmware7::configureUblox(std::shared_ptr<ublox_gps::Gps> gps)
{
  ...
  if (gps->poll(cfgGNSSRead)) {
    ...
    RCLCPP_DEBUG(
      node_->get_logger(), "Num. tracking channels in hardware: %i", cfgGNSSRead.num_trk_ch_hw);
    RCLCPP_DEBUG(
      node_->get_logger(), "Num. tracking channels to use: %i", cfgGNSSRead.num_trk_ch_use);
  } else {
  ...
  if (gnss_->isSupported("SBAS")) {
    ...
    block.flags =
      getRosBoolean(node_, "gnss.sbas") ? ublox_msgs::msg::CfgGNSSBlock::SIG_CFG_SBAS_L1CA : 0;
    cfgGNSSWrite.blocks[0] = block;
    if (!gps->configure(cfgGNSSWrite)) {
      throw std::runtime_error(
              std::string("Failed to ") +
...
void UbloxFirmware7::subscribe(std::shared_ptr<ublox_gps::Gps> gps)
{
  ...
  // from this)
  gps->subscribe<ublox_msgs::msg::NavPVT7>(
    std::bind(
  ...
  if (getRosBoolean(node_, "publish.nav.svinfo")) {
    gps->subscribe<ublox_msgs::msg::NavSVINFO>(
      [this](const ublox_msgs::msg::NavSVINFO & m) {nav_svinfo_pub_->publish(m);},
